...
Data = pd.read_csv('StudentData.csv')  #Read in CSV

before_processed_shape = Data.shape[0]
Data = Data.dropna()
after_processed_shape = Data.shape[0]
DiscardedCount = open("StackedEnsLRIndividualDiscardCount.txt", 'a')
...
Kf = KFold(n_splits= 5, shuffle= True, random_state= 444) # set splits to 5 so that folds are somewhat roportional to class size
Kf2 = KFold(n_splits= 5, shuffle= True, random_state= 926)
